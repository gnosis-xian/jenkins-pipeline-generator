...
node {
    try {
        ...
        if (maven_install) {
            stage('Maven Install') {
                sh "cd $project_child_path && $maven_home clean install -DskipTest=true  --settings=$maven_settings_file_path"
        ...
        if (maven_package) {
            stage('Compile') {
                sh "cd $project_child_path && $maven_home clean package -Dmaven.test.skip=true --settings=$maven_settings_file_path -P $env"
